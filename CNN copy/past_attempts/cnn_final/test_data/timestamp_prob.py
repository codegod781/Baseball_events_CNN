...
file_path = 'total_test_data.csv'  # Update this with the path to your test data file
...
probabilities_df.insert(0, 'time_trame', np.arange(0, len(predictions) * 0.25, 0.25))
...
for i, row in enumerate(predictions):
    ...
    if any(prob > 0.90 for prob in swing_probs):
        if has_supporting_probability(i, range(4, 7), 0.1):
            num_swings += 1
            increment_rows.append(f"Swing detected at row {i+2} time {(i+2)/4}")
            ignore_count = 10
        continue
    elif any(0.7 < prob <= 0.9 for prob in swing_probs):
        if has_supporting_probability(i, range(4, 7), 0.25):
            num_swings += 1
            increment_rows.append(f"Swing detected at row {i+2}  time {(i+2)/4} (low threshold)")
            ignore_count = 10
        continue
    ...
    if any(prob > 0.90 for prob in throw_probs):
        if has_supporting_probability(i, range(1, 4), 0.1):
            num_throws += 1
            increment_rows.append(f"Throw detected at row {i+2}  time {(i+2)/4}")
            ignore_count = 10
        continue
    elif any(0.7 < prob <= 0.9 for prob in throw_probs):
        if has_supporting_probability(i, range(1, 4), 0.25):
            num_throws += 1
            increment_rows.append(f"Throw detected at row {i+2}  time {(i+2)/4} (low threshold)")
            ignore_count = 10
        continue
